...
# ──────────────────────────────────────────────────────────────
# 2. Choose web server (Apache / Nginx)
# ──────────────────────────────────────────────────────────────
choose_webserver() {
    echo
    prompt "Select web server for native PHP (Docker+Sail is always available):"
    echo "   1) Apache"
    echo "   2) Nginx"
...
install_packages() {
    ...
    case $DISTRO in
        ...
        fedora)
            sudo dnf update -y
            sudo dnf install -y \
                docker docker-compose php php-json php-mbstring php-xml php-pdo php-mysqlnd \
                mariadb-server neovim nodejs npm fish \
                aircrack-ng clang llvm lld openssl elfutils-libelf \
                kernel-headers kernel-devel make gcc git curl wget
        ...
        debian)
            ...
            sudo apt install -y \
                docker.io docker-compose php php-cli php-mbstring php-xml php-bcmath php-zip php-mysql \
                mariadb-server neovim nodejs npm fish \
                aircrack-ng clang llvm lld openssl libelf-dev \
                linux-headers-$(uname -r) build-essential git curl wget ca-certificates gnupg
...
# ──────────────────────────────────────────────────────────────
# 7. Auto‑generate web‑server config (Apache or Nginx)
# ──────────────────────────────────────────────────────────────
generate_web_config() {
    local proj_dir="$1"
    ...
    if [[ $WEBSERVER == "apache" ]]; then
        local conf="/etc/httpd/conf.d/000-laravel.conf"   # Fedora/Arch
...
EOF
    ...
    log "Web‑server config generated and service restarted."
}
...
create_laravel_project() {
    ...
    # Generate web‑server config pointing to this project
...
final_instructions() {
    ...
    log "=== SETUP COMPLETE ==="
    echo
    echo "Web server: $WEBSERVER (auto‑configured for the Laravel project)"
    echo "Docker + Laravel Sail + Vite: fully functional"
    echo "Fish shell installed – run 'fish' to try it"
    echo
    echo "Next steps:"
